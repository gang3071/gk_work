...
class SAGameController
{
    ...
    public function bet(Request $request): Response
    {
        ...
        try {
            $params = $request->rawBody();
            $data = $this->service->decrypt($params);
            Log::channel('sa_server')->info('SA下注请求（Redis缓存）', ['params' => $data]);
            if ($this->service->error) {
                return $this->error($this->service->error);
            }
            ...
            Log::channel('sa_server')->info('SA下注成功（Redis缓存）', [
                'order_no' => $orderNo,
    ...
    public function cancelBet(Request $request): Response
    {
        ...
        try {
            $params = $request->rawBody();
            $data = $this->service->decrypt($params);
            Log::channel('sa_server')->info('SA取消下注请求（Redis缓存）', ['params' => $data]);
            if ($this->service->error) {
                return $this->error($this->service->error);
            }
            ...
            $elapsed = (microtime(true) - $startTime) * 1000;
            Log::channel('sa_server')->info('SA取消下注成功（Redis缓存）', [
                'order_no' => $orderNo,
                'elapsed_ms' => round($elapsed, 2),
            ]);
    ...
    public function betResult(Request $request): Response
    {
        try {
            ...
            Log::channel('sa_server')->info('SA结算成功（Redis缓存）', [
                'count' => count($betList),
                'elapsed_ms' => round($elapsed, 2),
            ]);
