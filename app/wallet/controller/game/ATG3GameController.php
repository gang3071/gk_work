...
class ATG3GameController
{
    ...
    public function balance(Request $request): Response
    {
        try {
            $params = $request->post();
            $this->service->decrypt(array_merge(['token' => $request->header('token'), 'timestamp' => $request->header('timestamp')], $params));
            ...
            $this->log->error('ATG3 balance failed', ['error' => $e->getMessage()]);
            $this->sendTelegramAlert('ATG3', '余额查询异常', $e, ['params' => $request->post()]);
    ...
    public function bet(Request $request): Response
    {
        try {
            ...
            $result = RedisLuaScripts::atomicBet($player->id, 'ATG3', $luaParams);

            // 审计日志
            logLuaScriptCall('bet', 'ATG3', $player->id, $luaParams);

            // 保存下注记录到 Redis（供 GameRecordSyncWorker 同步和推送）
            if ($result['ok'] === 1) {
                \app\service\GameRecordCacheService::saveBet('ATG3', [
                    'order_no' => $orderNo,
                    'player_id' => $player->id,
                    'platform_id' => $this->service->platform->id,
                    'amount' => $bet,
            ...
            // 游戏交互日志
            logGameInteraction('ATG3', 'bet', $data, [
                'ok' => $result['ok'],
            ...
            if ($result['ok'] === 0) {
                if ($result['error'] === 'duplicate_order') {
                    ...
                    // 余额不足
                    $this->log->warning('ATG3余额不足（Lua检测）', [
                        'order_no' => $orderNo,
                        'bet' => $bet,
                        'balance' => $result['balance'],
                    ]);
            ...
            $this->log->error('ATG3 bet failed', ['error' => $e->getMessage()]);
            $this->sendTelegramAlert('ATG3', '下注异常', $e, ['params' => $request->post()]);
            return $this->error(self::API_CODE_FAIL);
        }
    }
    ...
    public function betResult(Request $request): Response
    {
        try {
            ...
            $betAmount = getBetAmountWithFallback('ATG3', $orderNo, $player->id, $this->service->platform->id);
            ...
            $result = RedisLuaScripts::atomicSettle($player->id, 'ATG3', $luaParams);

            // 审计日志
            logLuaScriptCall('settle', 'ATG3', $player->id, $luaParams);

            // 保存结算记录到 Redis（供 GameRecordSyncWorker 同步和推送）
            if ($result['ok'] === 1) {
                \app\service\GameRecordCacheService::saveSettle('ATG3', [
                    'order_no' => $orderNo,
                    'player_id' => $player->id,
                    'platform_id' => $this->service->platform->id,
                    'amount' => max($winAmount, 0),
            ...
            $this->log->error('ATG3 betResult failed', ['error' => $e->getMessage()]);
            $this->sendTelegramAlert('ATG3', '结算异常', $e, ['params' => $request->post()]);
            return $this->error(self::API_CODE_FAIL);
        }
    }
    ...
    public function refund(Request $request): Response
    {
        try {
            ...
            $result = RedisLuaScripts::atomicCancel($player->id, 'ATG3', $luaParams);

            // 审计日志
            logLuaScriptCall('cancel', 'ATG3', $player->id, $luaParams);

            // 保存取消记录到 Redis
            if ($result['ok'] === 1) {
                \app\service\GameRecordCacheService::saveCancel('ATG3', [
                    'order_no' => $orderNo,
                    'player_id' => $player->id,
                    'platform_id' => $this->service->platform->id,
                    'refund_amount' => $refundAmount,
            ...
            $this->log->error('ATG3 refund failed', ['error' => $e->getMessage()]);
            $this->sendTelegramAlert('ATG3', '退款异常', $e, ['params' => $request->post()]);
            return $this->error(self::API_CODE_FAIL);
        }
    }
