...
class ATG2GameController
{
    ...
    public function bet(Request $request): Response
    {
        try {
            ...
            $result = RedisLuaScripts::atomicBet($player->id, 'ATG2', $luaParams);

            // 审计日志
            logLuaScriptCall('bet', 'ATG2', $player->id, $luaParams);

            // 保存下注记录到 Redis（供 GameRecordSyncWorker 同步和推送）
            if ($result['ok'] === 1) {
                \app\service\GameRecordCacheService::saveBet('ATG2', [
                    'order_no' => $orderNo,
                    'player_id' => $player->id,
                    'platform_id' => $this->service->platform->id,
                    'amount' => $bet,
            ...
            // 游戏交互日志
            logGameInteraction('ATG2', 'bet', $data, [
                'ok' => $result['ok'],
            ...
            if ($result['ok'] === 0) {
                if ($result['error'] === 'duplicate_order') {
                    ...
                    // 余额不足
                    $this->log->warning('ATG2余额不足（Lua检测）', [
                        'order_no' => $orderNo,
                        'bet' => $bet,
                        'balance' => $result['balance'],
                    ]);
            ...
            $this->log->error('ATG2 bet failed', ['error' => $e->getMessage()]);
            $this->sendTelegramAlert('ATG2', '下注异常', $e, ['params' => $request->post()]);
            return $this->error(self::API_CODE_FAIL);
        }
    }
    ...
    public function betResult(Request $request): Response
    {
        try {
            ...
            $betAmount = getBetAmountWithFallback('ATG2', $orderNo, $player->id, $this->service->platform->id);
            ...
            $result = RedisLuaScripts::atomicSettle($player->id, 'ATG2', $luaParams);

            // 审计日志
            logLuaScriptCall('settle', 'ATG2', $player->id, $luaParams);

            // 保存结算记录到 Redis（供 GameRecordSyncWorker 同步和推送）
            if ($result['ok'] === 1) {
                \app\service\GameRecordCacheService::saveSettle('ATG2', [
                    'order_no' => $orderNo,
                    'player_id' => $player->id,
                    'platform_id' => $this->service->platform->id,
                    'amount' => max($winAmount, 0),
            ...
            $this->log->error('ATG2 betResult failed', ['error' => $e->getMessage()]);
            $this->sendTelegramAlert('ATG2', '结算异常', $e, ['params' => $request->post()]);
            return $this->error(self::API_CODE_FAIL);
        }
    }
    ...
    public function refund(Request $request): Response
    {
        try {
            ...
            $result = RedisLuaScripts::atomicCancel($player->id, 'ATG2', $luaParams);

            // 审计日志
            logLuaScriptCall('cancel', 'ATG2', $player->id, $luaParams);

            // 保存取消记录到 Redis
            if ($result['ok'] === 1) {
                \app\service\GameRecordCacheService::saveCancel('ATG2', [
                    'order_no' => $orderNo,
                    'player_id' => $player->id,
                    'platform_id' => $this->service->platform->id,
                    'refund_amount' => $refundAmount,
