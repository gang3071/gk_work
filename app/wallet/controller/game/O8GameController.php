    ...
    public function bet(Request $request): Response
    {
        try {
            ...
            foreach ($orders as $order) {
                ...
                // Lua 原子下注（传入"元"，RedisLuaScripts 内部会转为"分"）
                $luaParams = [
                    'order_no' => $orderNo,
        ...
        try {
            ...
            foreach ($orders as $order) {
                ...
                if ($txtype == 560) {
                    ...
                    // Lua 原子取消（传入"元"，RedisLuaScripts 内部会转为"分"）
                    $luaParams = [
                        'order_no' => $orderNo,
                        'platform_id' => $this->service->platform->id,
                        'refund_amount' => $refundAmount,  // ✅ 传入"元"
                ...
                // ✅ Settle 操作：直接使用 O8 传入的 amt 字段（派彩金额）
                // O8 API 字段含义：
                // - amt: 派彩金额（返还给玩家的总金额，包括本金）
                // - turnover: 累计流水（统计用）
                // - ggr: 累计GGR（统计用）
                $settleAmount = $order['amt'] ?? 0;  // 派彩金额（元）
                ...
                $originalBetAmount = 0;
                if (\support\Redis::exists($betRecordKey)) {
                    // 🎯 单位转换：Redis存储的是"分"，转换为"元"用于计算 diff
                    $amountInCents = (int)\support\Redis::hGet($betRecordKey, 'amount');
                    $originalBetAmount = round($amountInCents / 100, 2);
                }
                ...
                // Lua 原子结算（传入"元"，RedisLuaScripts 内部会转为"分"）
                $luaParams = [
                    'order_no' => $orderNo,
                    'platform_id' => $this->service->platform->id,
                    'amount' => $settleAmount,  // ✅ 传入"元"
                    'diff' => $diffAmount,      // ✅ 传入"元"
                ...
                if ($result['ok'] === 1) {
                    \app\service\GameRecordCacheService::saveSettle('O8', [
                        ...
                        'player_id' => $player->id,
                        'platform_id' => $this->service->platform->id,
                        'amount' => $settleAmount,  // ✅ 传入"元"（虽然不会被使用，但保持一致）
                        'diff' => $diffAmount,      // ✅ 传入"元"
                        'game_code' => $order['gamecode'] ?? '',
                        'original_data' => $order,
                        'balance_before' => $result['old_balance'] ?? 0,
