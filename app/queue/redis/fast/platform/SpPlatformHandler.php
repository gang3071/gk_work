 ...
 * SP 平台处理器
 * 特殊逻辑：
 * - resultamount是净盈亏，不是派彩金额
 * - win = max(resultamount, 0)
 * - diff = resultamount（不是amount - bet）
 */
class SpPlatformHandler extends BasePlatformHandler
{
    ...
    public function processSettle(array $data, Player $player): void
    {
        ...
        $platformId = $params['platform_id'] ?? 1;
        $amount = (float)($params['amount'] ?? 0);
        $resultAmount = (float)($params['result_amount'] ?? $amount);  // SP特殊：净盈亏

        $this->log->info("SP: 处理结算", [
            'order_no' => $orderNo,
            'amount' => $amount,
        ...
        if ($betRecord) {
            // SP特殊逻辑：
            // - win = max(resultamount, 0)  派彩金额
            // - diff = resultamount  净盈亏（可能为负数）
            $betRecord->win = $amount;  // 已经是 max(resultamount, 0)
            $betRecord->diff = $resultAmount;  // ← 关键：直接使用resultamount，不计算！
            ...
            $record->bet = 0;
            $record->win = $amount;
            $record->diff = $resultAmount;  // ← 关键：使用resultamount
