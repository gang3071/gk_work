 ...
 * MT 平台处理器
 * 特殊逻辑：
 * - 结算加款判断：status !== 2 且 amount > 0 才加钱
 * - status=2：未中奖，不加钱
 * - status=3：中奖
 * - status=4：和局
 */
class MtPlatformHandler extends BasePlatformHandler
{
    // MT平台状态常量
    ...
    public function processSettle(array $data, Player $player): void
    {
        ...
        // 5. 发送彩金队列（MT不在彩金队列白名单中，默认不发送）
        // MT平台的needsLotteryQueue()返回true，但原代码中MT不在白名单
        // 这里保留逻辑，在主调用中会检查

        $this->log->info("MT: 结算处理成功", [
            'order_no' => $orderNo,
