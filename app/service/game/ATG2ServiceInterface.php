...
use app\wallet\controller\game\ATG2GameController;
use Exception;
use support\Cache;
use support\Log;
use WebmanTech\LaravelHttpClient\Facades\Http;

class ATG2ServiceInterface extends GameServiceFactory implements GameServiceInterface, SingleWalletServiceInterface
{
    use LimitGroupTrait;
    ...
    public function __construct(Player $player = null)
    {
        ...
        if (!empty($missingConfigFields)) {
            $this->log->error('ATG2 配置文件不完整，请检查 .env 文件', [
                'missing_fields' => $missingConfigFields,
                'config' => $config,
            ]);
            throw new GameException('ATG2 平台配置缺失，请检查 .env 文件中的 ATG2_* 配置项: ' . implode(', ', $missingConfigFields));
        }
    ...
    protected function getLimitRedConfig(): ?array
    {
        ...
        // 构建ATG2限红参数（ATG2使用营运账号：operator, key, providerId）
    ...
    private function checkPlayer()
    {
        ...
        if (empty($operator)) {
            $this->log->error('ATG2 平台配置错误：operator 为空', [
                'config' => $this->config,
    ...
    public function doCurl(string $url, array $params = [], string $mode = 'post')
    {
        ...
        if (empty($token)) {
            ...
            $this->log->info('ATG2 获取Token - 请求报文', [
                'url' => $tokenUrl,
                'headers' => [
            ...
            $this->log->info('ATG2 获取Token - 响应报文', [
                'url' => $tokenUrl,
            ...
            if (!$tokenResponse->ok()) {
                $this->log->error('ATG2 获取Token失败 - HTTP错误', [
                    'url' => $tokenUrl,
                    'status_code' => $tokenResponse->status(),
                    'response_body' => $tokenResponse->body(),
                    'operator' => $config['operator'],
                ]);
                throw new GameException('ATG2获取Token失败: HTTP ' . $tokenResponse->status());
            }

            $data = $tokenResponse->json();
            if (empty($data['data']['token'])) {
                $this->log->error('ATG2 获取Token失败 - 响应无token', [
                    'url' => $tokenUrl,
                    'response' => $data,
                    'operator' => $config['operator'],
                ]);
                throw new GameException('ATG2获取Token失败: 响应中没有token');
            }
            $token = $data['data']['token'];
            Cache::set($cacheKey, $token, 4 * 60);

            $this->log->info('ATG2 Token缓存成功', [
                'operator' => $config['operator'],
                'cache_key' => $cacheKey,
            ]);
        }
    ...
    {
        return ATG2GameController::API_CODE_INSUFFICIENT_BALANCE;
    }
    ...
    public function decrypt($data)
    {
        ...
        $tokenCacheKey = 'atg2:decrypt:' . md5($token);
        ...
        if (!$player) {
            return $this->error = ATG2GameController::API_CODE_PLAYER_NOT_EXIST;
        }
