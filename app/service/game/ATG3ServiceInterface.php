...
use app\wallet\controller\game\ATG3GameController;
use Exception;
use support\Cache;
use support\Log;
use WebmanTech\LaravelHttpClient\Facades\Http;

class ATG3ServiceInterface extends GameServiceFactory implements GameServiceInterface, SingleWalletServiceInterface
{
    use LimitGroupTrait;
    ...
    public function __construct(Player $player = null)
    {
        ...
        if (!empty($missingConfigFields)) {
            ...
            throw new GameException('ATG3 平台配置缺失，请检查 .env 文件中的 ATG3_* 配置项: ' . implode(', ', $missingConfigFields));
        }
    ...
    private function checkPlayer()
    {
        ...
        if (empty($operator)) {
            $this->log->error('ATG3 平台配置错误：operator 为空', [
                'config' => $this->config,
    ...
    public function doCurl(string $url, array $params = [], string $mode = 'post')
    {
        ...
        if (empty($token)) {
            ...
            if (empty($data['data']['token'])) {
                ...
                throw new GameException('ATG3获取Token失败: 响应中没有token');
            }
    ...
    {
        return ATG3GameController::API_CODE_INSUFFICIENT_BALANCE;
    }
    ...
    private function cacheUsernameOperatorMapping(string $username, string $operator): void
    {
        ...
        $cacheKey = "atg3:player_operator:{$username}";
        // 缓存24小时（玩家切换限红组的频率很低）
    ...
    private function optimizeConfigOrder(array $configs, ?string $preferredOperator = null): array
    {
        ...
        foreach ($configs as $config) {
            $operator = $config['operator'] ?? '';
            if ($operator) {
                $statsKey = "atg3:operator_stats:{$operator}";
    ...
    public function decrypt($data)
    {
        ...
        if (!$timestamp || time() >= $timestamp) {
            return $this->error = ATG3GameController::API_CODE_DECRYPT_ERROR;
        }

        // ✅ 优化2: 基于token缓存解密结果（利用重试机制token相同的特性）
        // 三方重试3次使用相同token，缓存可避免重复解密
        $tokenCacheKey = 'atg3:decrypt:' . md5($token);
        ...
        if ($usernameHint) {
            $operatorCacheKey = "atg3:player_operator:{$usernameHint}";
        ...
        if (!$player) {
            return $this->error = ATG3GameController::API_CODE_PLAYER_NOT_EXIST;
        }
        ...
        $platformCode = $this->platform->code ?? 'ATG3';
        ...
        if (!$playerLimitConfig || !isset($playerLimitConfig['operator']) || !isset($playerLimitConfig['key'])) {
            $this->log->info('ATG3 decrypt: 未配置限红组，使用配置文件 fallback', [
                'player_id' => $player->id,
            ...
            if (!empty($missingFields)) {
                $this->log->error('ATG3 配置文件不完整', [
                    'missing_fields' => $missingFields,
                ]);
                return $this->error = ATG3GameController::API_CODE_FAIL;
            }
