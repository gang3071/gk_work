...
class PokemonBall extends MachineServices implements BaseMachine
{
        ...
        switch ($cmd) {
            ...
            case substr(self::CMD_LIGHT_HOLES, 2, 2): // 亮灯洞口
                ...
                $this->log->info('[PokemonBall] 亮灯洞口', array_merge($baseLog, [
                    ...
                    'light_holes_grid' => $this->formatHoleGrid($holes),
                ]));
                return true;
    ...
    protected function parseHoleBitmask(string $hex): array
    {
        ...
        // 亮灯位映射表（位→位置→洞口）
        // 九宫格: 6 3 9 / 7 4 1 / 8 5 2
        // 0046 = bits 1,2,6 → 位置2,4,9
        // 80A9 = bits 0,3,5,7,15 → 位置1,3,5,6,7
        $bitMap = [
            0 => 6,   // bit 0 → 位置1 → 6号洞 (已确认)
            1 => 3,   // bit 1 → 位置2 → 3号洞 (已确认)
            2 => 7,   // bit 2 → 位置4 → 7号洞 (已确认)
            3 => 9,   // bit 3 → 位置3 → 9号洞 (已确认)
            4 => 5,   // bit 4 → 位置8 → 5号洞 (待验证)
            5 => 4,   // bit 5 → 位置5 → 4号洞 (已确认)
            6 => 2,   // bit 6 → 位置9 → 2号洞 (已确认)
            7 => 1,   // bit 7 → 位置6 → 1号洞 (已确认)
     ...
     * 将洞口数组格式化为九宫格字符串
     *
     * @param array $holes 洞口号数组
     * @return string 九宫格字符串
     */
