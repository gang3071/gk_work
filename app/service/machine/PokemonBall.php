...
class PokemonBall extends MachineServices implements BaseMachine
{
    ...
    protected function parseHoleBitmask(string $hex): array
    {
        ...
        // 位置: 123/456/789
        // 洞口: 639/741/852
        // 801B = bits 0,1,3,4,15 → 位置3,5,6,8,9 → 洞口 [1,2,4,5,9]
        $bitMap = [
            0 => 9,   // bit 0 → 位置3 → 9号洞 (已确认)
            1 => 2,   // bit 1 → 位置9 → 2号洞 (已确认)
            2 => 6,   // 待验证
            3 => 4,   // bit 3 → 位置5 → 4号洞 (已确认)
            4 => 1,   // bit 4 → 位置6 → 1号洞 (已确认)
            5 => 4,   // 待验证
            6 => 9,   // 待验证
            7 => 8,   // bit 7 → 位置7 → 8号洞 (已确认)
            15 => 5,  // bit 15 → 位置8 → 5号洞 (已确认)
        ];
