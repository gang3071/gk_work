...
class MachineOperationService
{
    ...
    private function sendRawCmdWithReply(array $params): array
    {
        ...
        // ⚠️ 小淞钢珠单向指令列表（不回复专用消息，但会立即触发心跳）
        $oneWayCommands = ['46CCB3', '46CCB4', '46CCBA']; // 清除外部按钮、故障排除、清除押得数值

        if (in_array($cmdNormalized, $oneWayCommands)) {
            Log::channel('machine_operations')->info('[sendRawCmdWithReply] 单向指令，等待心跳确认', [
                'machine_id' => $this->machine->id,
                'cmd' => $cmd,
            ...
            // 记录发送前的心跳版本号
            $beforeHeartbeatVersion = (int)($this->services->getActionVersion('46c0') ?: 0);

            // 记录发送前的状态（用于验证）
            $beforeState = [
                'external_open_count' => $this->services->external_open_count ?? null,
                'external_wash_count' => $this->services->external_wash_count ?? null,
                'score' => $this->services->score ?? null,
            ];
            ...
            Log::channel('machine_operations')->info('[sendRawCmdWithReply] 单向指令已发送，等待心跳', [
                'machine_id' => $this->machine->id,
                'cmd' => $cmdNormalized,
            ...
            $maxWait = min($timeout, 3); // 最多等待3秒（心跳应该立即到来）

            while (time() - $startTime < $maxWait) {
                $currentHeartbeatVersion = (int)($this->services->getActionVersion('46c0') ?: 0);
                ...
                if ($currentHeartbeatVersion > $beforeHeartbeatVersion) {
                    ...
                    Log::channel('machine_operations')->info('[sendRawCmdWithReply] 收到心跳确认', [
                        'machine_id' => $this->machine->id,
                        'cmd' => $cmdNormalized,
                        'wait_time' => time() - $startTime,
                        'heartbeat_version' => $currentHeartbeatVersion,
                        'after_state' => $afterState,
                    ]);

                    // 验证结果
                    $verified = $this->verifyOneWayCommandResult($cmdNormalized, $beforeState, $afterState);

                    return [
                        'success' => true,
                        'is_one_way' => true,
                        'cmd' => $cmdNormalized,
                        'message' => '单向指令执行成功（心跳已确认）',
                        'verified' => $verified,
                        'before_state' => $beforeState,
                        'after_state' => $afterState,
                        'wait_time' => time() - $startTime,
            ...
            // 超时但指令已发送
            Log::channel('machine_operations')->warning('[sendRawCmdWithReply] 单向指令超时（未收到心跳）', [
                'machine_id' => $this->machine->id,
                'cmd' => $cmdNormalized,
                'wait_time' => $maxWait,
            ]);
     ...
     * 验证单向指令执行结果（通过心跳数据）
     *
     * @param string $cmd 指令码（大写）
     * @param array $beforeState 发送前的状态
     * @param array $afterState 发送后的状态
     * @return bool 是否验证成功
     */
    private function verifyOneWayCommandResult(string $cmd, array $beforeState, array $afterState): bool
    {
        switch ($cmd) {
            case '46CCB3': // 清除外部按钮码表
